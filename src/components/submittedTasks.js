...
const cellContents = (row, labelId, hideIdColumn) => (
    ...
    <TableCell align="center" padding="normal">{row.submittedOn}</TableCell>
...
export default function SubmittedTasks(){
    ...
    if (userProfile === 'employer'){
        filterCriteria = (task) => task.assignee !== null && task.is_submitted && task.status === 'In review';
        noContentMessage = 'You have no tasks to review';
        heading = 'Tasks to Review';
        renderApprove = true;
        selectable = true;
    } else {
        filterCriteria = (task) => task.assignee === userID && task.is_submitted && task.status === 'In review';
