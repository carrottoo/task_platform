  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 403 && !data.errors) {
        return {
          permissionDenied:
            "You do not have the permission for the action requested.",
        };
      }
...
export const formatTaskData = (taskData, taskMapping) => {
  return taskData.map((task, index) => {
    ...
    formattedTask.assignee = task.assignee_name
      ? task.assignee_name
      : "Not assigned";
    formattedTask.submittedOn = task.submitted_on
      ? new Date(task.submitted_on).toLocaleDateString()
      : "N/A";
    formattedTask.approvedOn = task.approved_on
      ? new Date(task.approved_on).toLocaleDateString()
      : "N/A";
