...
export async function listRetrival(token, url, objectName, setSessionExpiredOpen, 
    setError, setLoading, filter=false, filterCriteria=null){
    ...
    try{
        ...
        return objectName === 'task'
            ? formatTaskData(data, taskMapping) 
            : data; 
...
export async function deleteObject(token, baseUrl, objectId, 
    alternativeErrorMessage, supplementPermissionMessage, successMessage, 
    setSessionExpiredOpen, setAlertMessage, setAlertSeverity, setShowAlert, navigate
){
    ...
    try{
        ...
        if (result.error){
            setAlertMessage(result.error.message ? result.error.message : alternativeErrorMessage);
            setAlertSeverity('error')
            setShowAlert(true);
            return;
        }
...
export async function setLikeStatus (token, url, object, body, updateBody, 
    setSessionExpiredOpen, setAlertMessage, setAlertSeverity, setShowAlert, 
    isLike = true, isProperty = false
){
    try{
        ...
        if (existingRecord) {
            ...
            apiResponseToLikeStatus(result, isLike ? 'Like' : 'Dislike', isLike ? 'liked' : 'disliked', setAlertMessage, setAlertSeverity, setShowAlert);
        } else {
...
function apiResponseToLikeStatus(result, action, pastParticiple,
    setAlertMessage, setAlertSeverity, setShowAlert
){
    if (result.error) {
        ...
        } else if (responseError.non_field_errors && responseError.non_field_errors.code === 'unique') {
